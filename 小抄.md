...
$$\|\mathbf{A}\|_F^2 = \text{tr}(\mathbf{A}^\top\mathbf{A}) = \sum_i \lambda_i(\mathbf{A}^\top\mathbf{A}) = \sum_i \sigma_i^2(\mathbf{A})$$

$$\|\mathbf{A}\|_2 = \sigma_{\max}(\mathbf{A}) = \sqrt{\lambda_{\max}(\mathbf{A}^\top\mathbf{A})}$$

$$\sigma_i(\mathbf{A}) = \sqrt{\lambda_i(\mathbf{A}^\top\mathbf{A})}$$

$$\text{谱分解} = \text{实对称矩阵的对角化，且特征向量正交}$$

$$\mathbf{A} = \mathbf{Q}\mathbf{D}\mathbf{Q}^\top = \sum_{i=1}^{n} \lambda_i \mathbf{q}_i \mathbf{q}_i^\top$$
